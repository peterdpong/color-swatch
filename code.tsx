...
class ntc {
  // This class is a rewritten TypeScript version of ntc.js (Name That Color)
  // https://chir.ag/projects/ntc/

  constructor() {
    let hexCode: string, rgb: number[], hsl: number[];

    this.colorNames.forEach((colorName: string[], index: number) => {
      hexCode = "#" + colorName[0];
      rgb = this.toRGB(hexCode);
      hsl = this.toHSL(hexCode);


      this.colorNames[index].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]);

    });

  }

  getNameFromHexCode(hexCode: string): ntcResult | undefined {
    hexCode = hexCode.toLocaleUpperCase();

    if (hexCode.length < 3 || hexCode.length > 7) {
      return undefined;
    }
    ...
    if (hexCode.length === 4) {
      hexCode = "#" + hexCode.substring(1, 1) + hexCode.substring(1, 1) + hexCode.substring(2, 1) + hexCode.substring(2, 1) + hexCode.substring(3, 1) + hexCode.substring(3, 1);
    }

    let rgb: number[] = this.toRGB(hexCode);
    let hsl: number[] = this.toHSL(hexCode);
  ...
  toRGB(hexCode: string): number[] {
    let r = parseInt('0x' + hexCode.substring(1, 3));
    let g = parseInt('0x' + hexCode.substring(3, 5));
    let b = parseInt('0x' + hexCode.substring(5, 7));
