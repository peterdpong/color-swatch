...
let ntc = {
  ...
  name: function(color) {
    ...
    for(var i = 0; i < ntc.names.length; i++)
    {
      ...
      ndf1 = Math.pow(r - parseFloat(ntc.names[i][2]), 2) + Math.pow(g - parseFloat(ntc.names[i][3]), 2) + Math.pow(b - parseFloat(ntc.names[i][4]), 2);
      ndf2 = Math.pow(h - parseFloat(ntc.names[i][5]), 2) + Math.pow(s - parseFloat(ntc.names[i][6]), 2) + Math.pow(l - parseFloat(ntc.names[i][7]), 2);
      ndf = ndf1 + ndf2 * 2;
      if(df < 0 || df > ndf)
      {
        df = ndf;
...
function handleHexCodeInput(hexCode: string) {
...
function Widget() {
  ...
  let colorMatch = ntc.name(hexCode);
  let rgb = ntc.rgb(hexCode);
  ...
  return (
    ...
    <Rectangle
     ...
     />
      ...
      >
          <Text fontSize={15} width={150}>{colorMatch[1].toString()}</Text>
          ...
          <Text fontSize={15}>{`${rgb[0]}, ${rgb[1]}, ${rgb[2]}`}</Text>
      </AutoLayout>
